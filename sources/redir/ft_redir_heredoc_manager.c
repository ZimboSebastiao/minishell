...
int	ft_handle(t_cmd *cmd, char *delimiter_token, t_shell *shell)
{
	...
	info = parse_heredoc_delimiter(delimiter_token);
	if (!info)
		return (1);
	if (info->expand)
		result = redir_heredoc_with_expand(cmd, info->delimiter, shell);
	else
		result = redir_heredoc_noexpand(cmd, info->delimiter);
